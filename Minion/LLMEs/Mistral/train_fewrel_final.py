...
class Manager(object):
    def __init__(self, config) -> None:
        super().__init__()
        self.config = config
        self.proxy = ProxyNCA(80, 4096, level = 2).cuda()
    ...
    def train_model(self, encoder_llm, training_data, seen_des, seen_relations, list_seen_des, is_memory=False):
        ...
        for i in range(epoch):
            ...
            for batch_num, (instance, labels, ind) in enumerate(data_loader):
                ...
                encoder_llm.ema_update()
                #target = encoder_llm(instance, is_slow = True)
                #encoder.init_queue(target, labels)
                target_rep_des = encoder_llm(batch_instance['input'], is_slow = True)
                encoder_llm.init_queue(target_rep_des, labels)
                ...
                loss2 = self.llm_moment.mutual_information_loss_cluster(hidden, rep_des, labels, temperature=0.05)  # Recompute loss2
                ...
                if flag == 0:
                    loss1 = self.llm_moment.contrastive_loss(hidden, labels, is_memory, des =rep_des)
                    
                    loss5 = self.proxy(hidden, target_classes).cuda()
                    
                    loss6 = self.contrastive_loss_fct(hidden, target_rep_des, encoder_llm.queue, labels, encoder_llm.queue_labels, self.config.device)

                    loss = loss1 + 2*loss2  + 0.5*loss5 + 0.5* loss6#+ 0.25*loss3 + 0.25*loss4
