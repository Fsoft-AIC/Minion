    ...
    def train_model(self, encoder, training_data, seen_des, seen_relations, list_seen_des, is_memory=False):
        ...
        for i in range(epoch):         
            for batch_num, (instance, labels, ind) in enumerate(data_loader):
                ...
                loss2 = self.moment.mutual_information_loss_cluster(hidden, rep_des, labels, temperature=0.05)  # Recompute loss2
                ...
                if flag == 0:
                    loss1 = self.moment.contrastive_loss(hidden, labels, is_memory, des =rep_des)

                    loss5 = self.contrastive_loss_fct(hidden, target_rep_des, encoder.queue, labels, encoder.queue_labels, self.config.device)
